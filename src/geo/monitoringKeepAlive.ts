...
const KEEPALIVE_CONFIG_VERSION = 'balanced-30s-v7';
...
export async function startMonitoringKeepAlive(): Promise<boolean> {
  ...
  try {
    ...
    await Location.startLocationUpdatesAsync(MONITORING_KEEPALIVE_TASK_NAME, {
      // Process keep-alive only. Opens are native geofence/BT; High+15s
      // made Samsung request GPS every 2s and still batch while stationary.
