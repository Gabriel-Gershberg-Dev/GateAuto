/**
 * Custom AES-128 block used by PalGate token derivation.
 * Ported from homebridge-palgate (JS port of pylgate).
 */

export const BLOCK_SIZE = 16;
export const KEY_SIZE = 16;
export const TOKEN_SIZE = 23;
export const TIMESTAMP_OFFSET = 2;

export const T_C_KEY = new Uint8Array([
  0xfa, 0xd3, 0x25, 0x72, 0x81, 0x29, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3a, 0xb4, 0x5a, 0x65,
]);

const S_BOX = [
  ,
];

const INVERSE_S_BOX = [
  ,
];

const RCON = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];

function galoisMul2(value: number): number {
  return value & 0x80 ? ((value << 1) ^ 0x1b) & 0xff : (value << 1) & 0xff;
}

/**
 * AES block transform.
 * Note: `isDecrypt` naming matches homebridge-palgate; for PalGate step1 pass true, step2 false.
 */
export function aesBlock(
  stateBytes: Uint8Array,
  keyBytes: Uint8Array,
  isDecrypt: boolean,
): Uint8Array {
  if (stateBytes.length !== BLOCK_SIZE || keyBytes.length !== KEY_SIZE) {
    throw new Error('State and/or key are not 16 bytes');
  }
  const state = new Uint8Array(stateBytes);
  const key = new Uint8Array(keyBytes);
  aesBlockInPlace(state, key, isDecrypt);
  return state;
}

function aesBlockInPlace(state: Uint8Array, key: Uint8Array, decrypt: boolean): void {
  if (decrypt) {
    for (let rnd = 0; rnd < 10; rnd++) {
      key[0] = (S_BOX[key[13]] ^ key[0] ^ RCON[rnd]) & 0xff;
      key[1] = (S_BOX[key[14]] ^ key[1]) & 0xff;
      key[2] = (S_BOX[key[15]] ^ key[2]) & 0xff;
      key[3] = (S_BOX[key[12]] ^ key[3]) & 0xff;
      for (let i = 4; i < KEY_SIZE; i++) {
        key[i] = (key[i] ^ key[i - 4]) & 0xff;
      }
    }
    for (let i = 0; i < BLOCK_SIZE; i++) {
      state[i] = (state[i] ^ key[i]) & 0xff;
    }
  }

  for (let rnd = 0; rnd < 10; rnd++) {
    if (decrypt) {
      for (let i = KEY_SIZE - 1; i > 3; i--) {
        key[i] = (key[i] ^ key[i - 4]) & 0xff;
      }
      key[0] = (S_BOX[key[13]] ^ key[0] ^ RCON[9 - rnd]) & 0xff;
      key[1] = (S_BOX[key[14]] ^ key[1]) & 0xff;
      key[2] = (S_BOX[key[15]] ^ key[2]) & 0xff;
      key[3] = (S_BOX[key[12]] ^ key[3]) & 0xff;
    } else {
      for (let i = 0; i < BLOCK_SIZE; i++) {
        state[i] = S_BOX[state[i] ^ key[i]];
      }
      let buf1 = state[1];
      state[1] = state[5];
      state[5] = state[9];
      state[9] = state[13];
      state[13] = buf1;

      const tmp1 = state[2];
      const tmp2 = state[6];
      state[2] = state[10];
      state[6] = state[14];
      state[10] = tmp1;
      state[14] = tmp2;

      const tmp = state[15];
      state[15] = state[11];
      state[11] = state[7];
      state[7] = state[3];
      state[3] = tmp;
    }

    if ((rnd > 0 && decrypt) || (!decrypt && rnd < 9)) {
      for (let i = 0; i < 4; i++) {
        const base = i * 4;
        if (decrypt) {
          const b1 = galoisMul2(galoisMul2(state[base] ^ state[base + 2]));
          const b2 = galoisMul2(galoisMul2(state[base + 1] ^ state[base + 3]));
          state[base] = (state[base] ^ b1) & 0xff;
          state[base + 1] = (state[base + 1] ^ b2) & 0xff;
          state[base + 2] = (state[base + 2] ^ b1) & 0xff;
          state[base + 3] = (state[base + 3] ^ b2) & 0xff;
        }
        const mix = state[base] ^ state[base + 1] ^ state[base + 2] ^ state[base + 3];
        const first = state[base];
        let buf3 = galoisMul2(state[base] ^ state[base + 1]);
        state[base] = (state[base] ^ buf3 ^ mix) & 0xff;
        buf3 = galoisMul2(state[base + 1] ^ state[base + 2]);
        state[base + 1] = (state[base + 1] ^ buf3 ^ mix) & 0xff;
        buf3 = galoisMul2(state[base + 2] ^ state[base + 3]);
        state[base + 2] = (state[base + 2] ^ buf3 ^ mix) & 0xff;
        buf3 = galoisMul2(state[base + 3] ^ first);
        state[base + 3] = (state[base + 3] ^ buf3 ^ mix) & 0xff;
      }
    }

    if (decrypt) {
      let buf1 = state[13];
      state[13] = state[9];
      state[9] = state[5];
      state[5] = state[1];
      state[1] = buf1;

      const tmp1 = state[10];
      const tmp2 = state[14];
      state[10] = state[2];
      state[14] = state[6];
      state[2] = tmp1;
      state[6] = tmp2;

      const tmp = state[3];
      state[3] = state[7];
      state[7] = state[11];
      state[11] = state[15];
      state[15] = tmp;

      for (let i = 0; i < BLOCK_SIZE; i++) {
        state[i] = (INVERSE_S_BOX[state[i]] ^ key[i]) & 0xff;
      }
    } else {
      key[0] = (S_BOX[key[13]] ^ key[0] ^ RCON[rnd]) & 0xff;
      key[1] = (S_BOX[key[14]] ^ key[1]) & 0xff;
      key[2] = (S_BOX[key[15]] ^ key[2]) & 0xff;
      key[3] = (S_BOX[key[12]] ^ key[3]) & 0xff;
      for (let i = 4; i < KEY_SIZE; i++) {
        key[i] = (key[i] ^ key[i - 4]) & 0xff;
      }
    }
  }

  if (!decrypt) {
    for (let i = 0; i < BLOCK_SIZE; i++) {
      state[i] = (state[i] ^ key[i]) & 0xff;
    }
  }
}

export function packUint64BE(num: number): Uint8Array {
  const result = new Uint8Array(8);
  let n = num;
  for (let i = 7; i >= 0; i--) {
    result[i] = n % 256;
    n = Math.floor(n / 256);
  }
  return result;
}

export function bytesToHex(bytes: Uint8Array): string {
  let hex = '';
  for (let i = 0; i < bytes.length; i++) {
    hex += bytes[i].toString(16).padStart(2, '0');
  }
  return hex;
}

export function hexToBytes(hex: string): Uint8Array {
  const clean = hex.trim().replace(/^0x/i, '');
  if (clean.length % 2 !== 0) {
    throw new Error('Invalid hex string');
  }
  const out = new Uint8Array(clean.length / 2);
  for (let i = 0; i < out.length; i++) {
    out[i] = parseInt(clean.slice(i * 2, i * 2 + 2), 16);
  }
  return out;
}
